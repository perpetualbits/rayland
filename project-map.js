  ...
  nodes: [
    ...
    },
    {
      ...
      parts: [
        ...
        { label: "Doorbell lost-wakeup (root-caused, fix pending)", status: "active", desc: "virglrenderer's ring thread waits at a blob-backed vkAllocateMemory for the blob resource; that blob arrives on the INLINE vtest path, but S rings the doorbell only after a ring delta — so nothing wakes the waiting thread, head freezes, and Venus aborts the spinning app silently (every Venus abort logs at MESA_LOG_DEBUG, which release Mesa suppresses). vkcube's first two swapchain images survive because later commands ring the doorbell incidentally; the third and last allocation has nothing following it. Measured: 91 doorbells, none missing/refused, all nine blobs created on S. Predates the blob-sync fix (A/B: identical backtrace at 3a7fc39)." }
      ],
