  ...
  nodes: [
    ...
    },
    {
      ...
      parts: [
        ...
        { label: "Ring thread parks and is never woken (open)", status: "active", desc: "SYMBOLISED (PIE page-offset trick on the stripped virgl_render_server, no debuginfo needed): the stalled thread is vkr_ring_thread itself, parked in its own idle cnd_wait on ring->cond — a thread trampoline calls it directly, so there are no dispatch frames. The park is legitimate (it only parks when cur == tail, and S's control words confirm it was caught up at 22704). S then applies tail -> 22820, rings the doorbell, virglrenderer accepts it, the context is NOT fatal (FATAL never set) so the ring lookup succeeded — and head never moves. vkcube spins ~4.5s and Venus aborts it silently (every Venus abort logs at MESA_LOG_DEBUG, which release Mesa suppresses). Six readings of this stall; every inference died, every measurement survived. Refuted: delayed ACK, reply-decode overrun, doorbell lost-wakeup, buried multi-ring creation (one ring only — two 'vkr-ring-1' threads are NVIDIA workers inheriting the comm name), fatal context, and blocked-inside-dispatch. Predates the blob-sync fix (A/B: identical backtrace at 3a7fc39). NEXT: read the ring's pending_notify flag to see whether vkr_ring_notify actually runs for that doorbell." }
      ],
