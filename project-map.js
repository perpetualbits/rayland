  ...
  nodes: [
    ...
    /* ----------------------------------------------------------------- S-side layer */
    {
      ...
      parts: [
        ...
        { label: "WP0 4.5: the app's OWN window on S's screen", status: "active", desc: "HALF REACHED 2026-08-29. An unmodified vkcube on apollo, drawn by dop561's GPU, appears in its own window on dop561's real compositor -- the app's own Wayland session replayed, not a picture of it, and no pixels crossing the network. Photographed: docs/data/2026-08-29-wp0-event-witness/cube-on-dop561.png. But it is a STILL FRAME: the 450x450 interior was pixel-identical, 0 of 202500 differing, across two captures 17 s apart, in each of two runs. STILL not spinning after the recycled-id fix, and the wall has MOVED UPSTREAM. Five runs: four got ONE frame with S's compositor emitting ZERO wl_surface.frame callbacks at all; the fifth got nine, then the app stopped ASKING (no tenth frame request) having seen only 2 wl_buffer.release for 9 attaches. Both shapes point outside anything Rayland relays: a compositor emits a frame callback only when it actually composites the surface, and releases a buffer only when it stops using one. The variance fits window placement/visibility, and the one run that animated is the one whose window was photographed on screen. NEXT OBSERVATION NEEDED: in a one-frame run, is there no window, or a window present but never composited? cosmic-screenshot went unresponsive partway through the session, so that may need a human at the screen. Also seen: wl_keyboard.keymap is dropped because it carries an fd (no relayed app will have a keymap until that gets a token-style substitution), and 960 of the 1004 events S's compositor emitted were the deliberately suppressed dmabuf format/modifier pair." },
