  ...
  nodes: [
    ...
    },
    {
      ...
      parts: [
        ...
        { label: "C's last delta never lands on S (open)", status: "active", desc: "Periodic sampling of S's ring words (not just on delta apply — the delta-driven log goes silent exactly when the stall starts) shows the CONSUMER IS FINE: head advances past the vkAllocateMemory the app blocks after, and the ring thread wakes on the doorbell and re-parks normally. The freeze point also varies run to run, so it was never a property of one command. What is invariant: C sends N ring messages and S applies N-1 — the missing one is always the LAST, carrying the command the app awaits a reply to — after which the app spins ~4.2s and Venus aborts it silently (every Venus abort logs at MESA_LOG_DEBUG, which release Mesa suppresses). Refuted by measurement: delayed ACK, reply-decode overrun, doorbell lost-wakeup, multi-ring (one ring; two 'vkr-ring-1' threads are NVIDIA workers inheriting the comm name), fatal context, blocked-in-dispatch, and consumer stall. Predates the blob-sync fix (A/B: identical backtrace at 3a7fc39). NEXT: log every message S reads and writes — 'sent' and 'applied' are currently the only observable points on a path with several steps between them." }
      ],
