  ...
  nodes: [
    ...
    },
    {
      ...
      parts: [
        ...
        { label: "Deadlock inside rayland-s (located)", status: "active", desc: "LOCATED, and it was never Mesa or virglrenderer. Logging every message S reads/writes shows S RECEIVES the final delta ('r< RingDelta tail=22900', the command the app awaits) and then BOTH its threads stop at the same instant: no apply_delta, no further periodic ring samples (they run 4x/sec through a ~4s stall), no further writes. Writes are all matched (4676 w> / 4676 w<), so nothing was left unflushed. The message thread holds the applier lock across apply() — including its engine calls — and takes the send lock inside that scope; its own comment argues no deadlock from the two-lock cycle but rests on the actor 'servicing promptly', which is an assumption, not an invariant. Caveats kept honest: the message thread stopped BEFORE apply_delta (which touches no engine), and the periodic sampler runs with the applier lock held, so the instrument cannot yet be excluded. Refuted along the way: delayed ACK, reply-decode overrun, doorbell lost-wakeup, multi-ring, fatal context, blocked-in-dispatch, consumer stall, unflushed send. NEXT: try_lock with logged waits to name holder and waiter, plus a run with the sampler off." }
      ],
