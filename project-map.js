  ...
  nodes: [
    ...
    /* ----------------------------------------------------------------- S-side layer */
    {
      ...
      parts: [
        ...
        { label: "Wayland replay", status: "active", desc: "Rebuilds the app's object graph on S's real compositor — WP0. As of 2026-08-29 it also ORIGINATES the buffer-creation sequence for a relayed BufferToken (4.3), the one thing it does not merely translate. THE STALL IS NOW LOCATED (2026-08-29) and it is NOT in this crate. The app never gets its SECOND wl_surface.frame callback, so it never draws again. Found by instrumenting both halves of the event return path rather than reasoning about them -- translate_and_emit had four bare returns that logged nothing, so the previous report's 'no S-side replay errors' was true of a path that was silent by construction. S is fully exonerated: it emitted wl_callback.done TWICE and mapped both callbacks consistently. The loss is a RECYCLED-ID RACE in rayland-c's proxy: a wl_callback.done is a DESTRUCTOR event, libwayland reuses the id it just freed for the next callback, and ProxyState::objects is keyed by bare protocol_id -- so destroyed() for callback #1 runs AFTER callback #2 was registered under the same number and deletes it. Evidence in docs/data/2026-08-29-wp0-event-witness/. NOT fixed, by instruction; the fix is to remove only when the stored ObjectId is still the object being destroyed. rayland-s's IdMaps.reverse is keyed the same way and is accidentally safe only because its destroyed() is a no-op." },
        ...
        { label: "WP0 4.5: the app's OWN window on S's screen", status: "active", desc: "HALF REACHED 2026-08-29. An unmodified vkcube on apollo, drawn by dop561's GPU, appears in its own window on dop561's real compositor -- the app's own Wayland session replayed, not a picture of it, and no pixels crossing the network. Photographed: docs/data/2026-08-29-wp0-event-witness/cube-on-dop561.png. But it is a STILL FRAME: the 450x450 interior was pixel-identical, 0 of 202500 differing, across two captures 17 s apart, in each of two runs. Not spinning, because the app never receives its second frame callback -- see the Wayland replay node for the located cause. Also seen: wl_keyboard.keymap is dropped because it carries an fd (no relayed app will have a keymap until that gets a token-style substitution), and 960 of the 1004 events S's compositor emitted were the deliberately suppressed dmabuf format/modifier pair." },
