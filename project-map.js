  ...
  nodes: [
    ...
    /* ----------------------------------------------------------------- S-side layer */
    {
      ...
      parts: [
        ...
        { label: "Wayland replay", status: "active", desc: "Rebuilds the app's object graph on S's real compositor — WP0. As of 2026-08-29 it also ORIGINATES the buffer-creation sequence for a relayed BufferToken (4.3), the one thing it does not merely translate. NEXT WALL, seen on the 4.3 run and NOT chased: vkcube attaches, damages, requests a frame callback and commits ONCE, then stalls alive -- main thread on a futex held by Venus's own vn_wsi[0,0] thread, which sits in poll, with 12 events delivered back. It is waiting for presentation feedback of some kind and which event is unknown. Ruled out: the token path (buffers exist, compositor took them), S-side replay errors (none logged), and the missing commit gate (which would tear or run early, not go silent). IdMaps::insert populates both directions for the synthesized objects, so a wl_buffer.release could route back. 64 proxy trace lines against the 36 of 2026-07-25. 4.5 NOT reached." },
