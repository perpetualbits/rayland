  ...
  nodes: [
    ...
    },
    {
      ...
      parts: [
        ...
        { label: "Ring consumer stalls on S (open, 4 theories refuted)", status: "active", desc: "head freezes at 22704 while C relays on to 22900; vkcube spins ~4.5s and Venus aborts it silently (every Venus abort logs at MESA_LOG_DEBUG, which release Mesa suppresses). MEASURED: S applies the delta, rings the doorbell (91 rung, none missing or refused, one correct handle), and creates all nine blobs — and every vkr-ring thread in the virgl-1-gpu_ren worker is parked in futex_do_wait. Refuted: delayed ACK, reply-decode overrun, doorbell lost-wakeup (virglrenderer never waits for a blob — it errors and returns), and buried multi-ring creation (exactly one vkCreateRingMESA, at offset 0, correctly latched). Predates the blob-sync fix (A/B: identical backtrace at 3a7fc39). NEXT: gdb the parked worker for its stack." }
      ],
