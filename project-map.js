  ...
  nodes: [
    ...
    /* ----------------------------------------------------------------- S-side layer */
    {
      ...
      parts: [
        ...
        { label: "Measured reliability: 0 failures in 480 real-network runs", status: "done", desc: "2026-07-27. Until then the strongest statement this project could make about itself was '10/10 clean', so a single observed failure had nothing to be read against -- and one was briefly (and wrongly) taken as condemning Venus's feedback mechanisms. An overnight soak (scripts/soak-failure-rate.sh, preserved in the tree) ran 400 real-network icosa-cpu sessions of the SHIPPING configuration: 0 failures, verified per-attempt rather than from the summary line. With earlier sweeps that is 0/480, i.e. a rate under 0.62% at 95% (rule of three). This does NOT separate the feedback arms statistically -- 1 event against 0 proves nothing, Fisher p=0.53 -- but it establishes the floor a single loss must now be read against. THE CHEAPEST QUEUED EXPERIMENT is the same harness with VN_PERF_SETTING=no_multi_ring,no_fence_feedback for 400 runs: that arm measured 1.23x faster and carries exactly one unexplained failure (1/92). One night settles it." },
        { label: "Teardown SIGABRT (FIXED)", status: "done", desc: "2026-07-27. ~21% of session teardowns ended in SIGABRT rather than a clean exit (83/400 in the soak; 2/10 in a targeted hunt). Caught only by keeping EVERY S log instead of overwriting it per iteration -- the message had been produced on every one of those aborts and destroyed each time by the next run. It is libepoxy: `epoxy_get_proc_address: Assertion 0 && \"Couldn't find current GLX or EGL context\"`, reached from virgl_renderer_cleanup inside VirglEngine::drop as it releases the EGL winsys -- a GL entry point resolved with no current context, and epoxy's answer is abort(). Intermittent because it depends on thread and subprocess wind-down order, which is why one clean run looked like a refutation. FIX: the daemon exits via process::exit(0) without running that Drop. Drop is untouched for embedders and tests, where repeated new->use->drop cycles make its ordering load-bearing; for an exiting process it buys nothing the kernel does not already do, and the render-server subprocess exits with its socket (demonstrated -- the runs that aborted skipped this same cleanup and left none behind). 0 aborts in 16 sessions vs ~3-4 expected. Mattered because an abort on a fifth of teardowns is a standing false positive in front of every future crash investigation." },
        { label: "WP0 4.3 part 1: exported dma-buf descriptor retained", status: "done", desc: "2026-07-27. The app's swapchain images are HOST3D blobs whose device memory virglrenderer exports as a dma-buf ONCE, at creation (mem->exported forbids a second export). S was dropping that descriptor at the end of the creation scope -- harmless while nothing needed it, since S reaches every blob through its own mmap, and fatal for presentation, which must hand exactly that fd to S's compositor. Applier now retains it keyed by resource id, releases it in UnrefResource, and exposes exported_fd() -> BorrowedFd so callers cannot take ownership of something the export cannot reproduce. Teeth-checked. ALSO FOUND: C's half of 4.3 was already complete -- wayland_proxy.rs's 'next sub-step' comments are stale, create_immed already assembles and forwards the full BufferToken -- so 4.3 is S-side only." },
        { label: "WP0 4.3 part 2: token -> wl_buffer (DONE, verified over the real network)", status: "done", desc: "LANDED 2026-08-29. S originates a THREE-request sequence against the dma-buf it exported at resource creation: create_params on the bound dmabuf global, add(fd, plane 0, offset, stride, mod_hi, mod_lo) with the layout carried on the token, create_immed -> wl_buffer, each mapped into the app_id<->s_id table so the app's own attach/commit replay through the existing path. Reached through the ExportedFdSource trait, which makes the lock rule structural: the applier guard cannot escape dup_exported_fd, so no caller can hold it across a compositor round trip. MEASURED over the real network (scripts/wp0-vkcube-two-machine.sh, apollo -> dop561, ONE run): 4 of 4 swapchain images became real wl_buffers, 500x500 XRGB8888 LINEAR, NO protocol error -- and since create_immed is the immediate variant, which raises a FATAL protocol error on a bad buffer, silent acceptance is positive evidence the dma-buf actually imported. TWO PLAN GAPS FOUND. (1) The plan was short by a whole request: C intercepts create_params and never forwards it, so S has no params object and the app-side params id is unmapped -- a faithful implementation is refused at the sender lookup before it ever sees the token; the token check therefore happens BEFORE that lookup. (2) The plan said child_spec wl_buffer v1; a Wayland child INHERITS ITS PARENT'S VERSION and wayland-backend panics unless child_spec matches the sender (rs/client_impl/mod.rs:367). The first two-machine run died on 'expected version 3 but got 1' and took the daemon's MAIN THREAD with it, because the synthesis path lacked the catch_unwind the generic replay path has. Both fixed; the planner test now asserts the inheritance rather than the constant, teeth-checked by three mutations. Old note: Design is exact; see DIARY.md 2026-07-27. TWO DETAILS THE PLAN MISSES. (1) S must SYNTHESIZE the params.add rather than replay it: C intercepts add and drops the fd by design (that is buffer-by-token), so S's params object has no planes when create_immed arrives. That is a request S originates rather than translates -- a first for the replay module. (2) RESOLVED 2026-08-29: BufferToken now carries stride AND offset, taken verbatim from the app's params.add, because deriving width x bpp garbles pixels rather than failing cleanly and C knows the true value (it originates in the layout Venus queried on S's own GPU). Shipped with it: C REFUSES multi-plane buffers rather than approximating them -- a second add, or a plane_idx != 0, poisons the params object so create_immed forwards nothing. plane_idx is deliberately NOT carried: a token describes exactly one plane by construction. Tests teeth-checked by mutation (derive the stride -> the token test fails; drop the poison rule -> both refusal tests fail). Sequence: resolve token.resource_id via Applier::exported_fd, send_request an add (opcode 1) on the S-side params object with [Fd, Uint(0), Uint(0), Uint(stride), Uint(mod_hi), Uint(mod_lo)], then let the existing path replay create_immed. Resolve and clone the fd UNDER the applier lock and release it before any send_request -- a Wayland call under that lock puts the relay's mutex behind a compositor round trip. Not written because it cannot be verified without vkcube reaching attach (needs compositor + GPU + --gpu_number 0), and code that compiles while exercising no test would look done." },
        { label: "Synchronous round trip (THE remaining limit, now measured)", status: "seam", desc: "2026-07-26 located frame time by elimination rather than assertion. On LOOPBACK icosa-gpu costs ~50 ms/frame with a 78 KB return path — so not bandwidth. NOT message count either: batching ship()'s per-message lock and flush (29414 of each per 120-frame run) is worth 1.03x, measured from the fixture's own microsecond CSV. And the readback is NOT fragmented — res=5 averages 377-byte runs, its gap-256 coalescing already working; the one-byte flood is the reply arena (res=2, 3247 single-byte msgs) whose gap-0 grain is a DELIBERATE correctness property, since a gap byte is one S did not write and could clobber C's. What is left: with feedback off the app implements vkWaitForFences by polling vkGetFenceStatus, and every poll is a full C->S->execute->reply->C cycle. BOTH OBVIOUS ESCAPES ARE CLOSED. (a) Fence feedback would remove the polls and is structurally incompatible with (c)2's completion barrier, which works by spotting that very poll's VK_SUCCESS reply — enabling it gives exit 134 and 0 frames, every time. (b) Semaphore/event/query feedback measured 1.23x on loopback with all 120 frames bit-identical AND THEN FAILED the two-machine sweep: 9/10 clean, one run lost entirely to a silent Venus SIGABRT. They are shared status pages S writes and C's Mesa reads directly, which (c)1 does not relay (noted in c1-two-machine.sh for weeks; this session read that as unexplored rather than as a finding already paid for, and rediscovered it at the cost of a sweep). NOT ADOPTED — the sweep script keeps the setting parameterised but defaults to the value that passes. THE NEXT REAL MOVE IS RELAYING THE FEEDBACK PAGES, a (c)1 transport task, not a tuning knob. Method note: a loopback pass proved nothing here and proved it convincingly." },
        ...
        { label: "Wayland replay", status: "active", desc: "Rebuilds the app's object graph on S's real compositor — WP0. As of 2026-08-29 it also ORIGINATES the buffer-creation sequence for a relayed BufferToken (4.3), the one thing it does not merely translate. RECYCLED-ID RACE FOUND AND FIXED (2026-08-29). The app was never getting its SECOND frame callback. A wl_callback.done is a DESTRUCTOR event, libwayland reuses the id it just freed for the next callback, and ProxyState::objects was pruned by bare protocol_id -- so callback #1's LATE destroyed() deleted callback #2's entry and #2's done was dropped as unknown-object. A protocol id is a SLOT NUMBER, not an object identity. destroyed() now removes only when the entry still holds the object being destroyed (ObjectId equality includes a per-client serial); PendingParams got an owner field for the same reason, since its values are not ObjectIds. Guarded by tests/wayland_proxy_recycled_id.rs, which drives 30 callback cycles and fails 10/10 against remove-by-number (one cycle failed only 2/10 -- a single sample of a race is a coin flip). VERIFIED over the real network: zero drop:unknown-object in every run, 9 wl_callback.done emitted and 9 delivered. CORRECTION to the earlier note here: rayland-s's IdMaps is NOT an 'accidentally safe latent twin' -- insert() writes both directions at creation so a recycled id is refreshed by its new owner, nothing removes by number, and its no-op destroyed() is LOAD-BEARING. Adding a removal there would import this bug. Evidence: docs/data/2026-08-29-wp0-recycled-id-fix/." },
        { label: "Out-of-line command streams (vkExecuteCommandStreamsMESA)", status: "done", desc: "DELIVERED 2026-07-26. Venus replaces any submission over direct_size (buffer_size >> 4, so 8 KiB for the 128 KiB ring) with vkExecuteCommandStreamsMESA naming OTHER shmems. Those live in the staging pool, blob_id == 0, which the C->S sync skipped -- so S held zeros and rayland-c REFUSED the delta. Correct, but it made every submission over 8 KiB unrelayable, which is most real workloads. THE FIX REMOVES THE QUESTION RATHER THAN ANSWERING IT: messages_for_delta now carries every blob C holds EXCEPT the ring (which keeps RingDelta, carrying the tail that validates its bytes, sent last), so the referenced streams are already on S. The alternative -- making the refusal precise with the decoder -- was rejected because this is the RELAY path and (c)1 §7 forbids a decode from deciding what crosses the wire. Publishing a region S also writes is safe via the BASELINE, not blob_id: note_s_wrote folds each S->C write into it so S's replies are never echoed back; two teeth-checked tests state the surviving properties (ring never shipped as BlobData, arena never echoed). PREREQUISITE, anticipated rather than measured after the fact: take_changed_runs was byte-at-a-time, ruinous pointed at 8 MiB, so it was made memcmp-shaped first -- proved equivalent to a naive reference over nine patterns on and across the 64-byte boundaries, teeth-checked. RESULT: rayland-icosa-gpu (fractal in SPIR-V, evaluated by S's GPU) runs over the relay, 120/120 frames BIT-IDENTICAL to native on S, at ~41 ms/frame over the real link against icosa-cpu's 283 -- ~7x, ~24 fps. The forward mapped-write volume WAS the dominant cost; the return path (256 KiB/frame fragmenting into ~5000 one-byte messages) is now the bottleneck and the next target. Regression: two-machine icosa-cpu sweep still 10/10, 0 stale frames. NOTE it does NOT unblock vkcube -- that stream has zero type-180 commands and never met this refusal." },
        { label: "WP0 4.5: the app's OWN window on S's screen", status: "done", desc: "REACHED 2026-08-29. An unmodified vkcube on apollo, drawn by dop561's GPU, appears in its own window on dop561's real compositor -- the app's own Wayland session replayed, not a picture of it, and no pixels crossing the network. Photographed: docs/data/2026-08-29-wp0-event-witness/cube-on-dop561.png. But it is a STILL FRAME: the 450x450 interior was pixel-identical, 0 of 202500 differing, across two captures 17 s apart, in each of two runs. REACHED 2026-08-29, confirmed by a human watching the screen: an unmodified vkcube on apollo, drawn by dop561's GPU, SPINNING in its own window on dop561's screen -- and with pixels no longer crossing the network. Two fixes got there. (1) WaylandReplay cached the S-side ObjectId of the zwp_linux_dmabuf_v1 global at the moment the APPLICATION bound it; the app binds that global repeatedly while probing formats (12 times in one run) and destroys each, so the cached id named a dead object, every later create_params failed Invalid ObjectId, attach failed, and a wl_surface with no valid buffer is unmapped BY DEFINITION -- the window vanished while the app lived on. S now binds its OWN dmabuf global once and never destroys it. (2) THE PIXEL STREAM: measured 105,254,034 bytes S->C over ~60s / ~120 frames, i.e. ~877 KB per frame against a 1,000,000 byte frame -- S was shipping every rendered frame back to a machine with no display. The (c)2 return path ships whatever S's GPU wrote into any blob, correct for a readback and indistinguishable from a swapchain image the app only shows; only the WP0 token path knows which is which. Presented resources are now excluded exactly as rings are: S->C fell to 184,311 bytes, ~1.9 KB/frame, 571x less. The display was ALREADY zero-copy, so the waste had no symptom and every test passed -- it was found because the owner asked whether pixels were crossing the wire. NOT claimed: any frame rate, failure rate, or that presentation is paced/tear-free; the commit gate is still untouched. Also seen: wl_keyboard.keymap is dropped because it carries an fd (no relayed app will have a keymap until that gets a token-style substitution), and 960 of the 1004 events S's compositor emitted were the deliberately suppressed dmabuf format/modifier pair. MEASURED 2026-08-30 (scripts/wp0-soak.sh, 60 runs against headless weston): 59/60 clean, and the single failure is an artefact of the failure DEFINITION -- two events dropped during the app's final teardown, for objects it had legitimately destroyed -- so 0 genuine defects in 60. Throughput 261-489 attaches per 20 s run (median 438) = 13-24 fps. Traffic A/B'd inside one binary: S->C 307,776 -> 219 B/frame (1,406x); C->S 3,723 -> 3,594 B/frame (1.04x), so the unexplained C->S rise of 2026-08-29 was NOT an effect -- it compared a 120-frame run against a 96-frame one. The recycled-id fix is load-bearing: 470 stale destroys declined in one 20 s run. TWO DEFECTS were found by running a SECOND application (vkgears) and BOTH ARE FIXED 2026-08-30. (1) A bind capped on S was not propagated to its children -- IdMaps now records every object's version, seeded with the capped value at bind and inherited by each child, and child_spec is built from the SENDER rather than from the wire. That closes the version-inheritance rule as a CLASS after three instances (create_immed's wl_buffer, the params object, get_xdg_surface). (2) The catch_unwind around send_request cannot save the session -- the poisoned mutex is wayland-backend's OWN ConnectionState::protocol, held across every panic send_request raises, and lock_protocol() unwraps it -- so it now declares the replay DEAD, stops all backend calls (including the flush, and the compositor-reader thread), and claims no recovery. RESULT: vkgears now RUNS end to end -- 345 attaches, 345 frame callbacks, 10-13 fps, zero panics, two cappings chaining correctly through two generations. Guarded soak 25/25 clean. rayland-icosa-window cannot run over WP0 and refuses cleanly: it is a wl_shm client (fd-passing, pixel-carrying) and WP0 is a dmabuf mechanism." },
