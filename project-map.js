  ...
  nodes: [
    ...
    },
    {
      ...
      parts: [
        ...
        { label: "Consumer blocked inside vkAllocateMemory (open)", status: "active", desc: "virglrenderer's ring thread enters the vkAllocateMemory dispatch at ring offset 22704 and never returns: S's own ring status reads ALIVE without IDLE (so it is NOT in the idle park), FATAL is never set (so the context is healthy and the doorbell finds its ring), and head stays frozen while C relays on to 22900. vkcube spins ~4.5s and Venus aborts it silently — every Venus abort logs at MESA_LOG_DEBUG, which release Mesa suppresses. Five theories refuted by measurement: delayed ACK, reply-decode overrun, doorbell lost-wakeup, buried multi-ring creation (only one ring exists — two 'vkr-ring-1' threads are NVIDIA workers that inherited the comm name), and a fatal context. Predates the blob-sync fix (A/B: identical backtrace at 3a7fc39). NEXT: symbolise the blocked frames against a debug virglrenderer 1.2.0 to name the condvar." }
      ],
