...
window.PROJECT_MAP = {
  ...
  nodes: [
    ...
    /* ----------------------------------------------------------------- wire layer */
    {
      ...
      desc: "IN PROGRESS (Task 4/N landed). Rayland can relay Venus streams byte-exactly and, until Task 4, could not read them: encoded_size can only express FIXED encodings, so the decoder used to halt at the application's first real command and 'where does this command end' could not be asked — which is what blocked the open vkQueueSubmit framing question. This crate borrows Mesa's own generated venus-protocol (73k lines, 43 headers) behind a C shim, rather than reimplementing it, for the same reason CLAUDE.md reuses the rendering engine: a second source of truth for a format Rayland does not own will diverge, and the symptom is a decoder confidently reporting the wrong thing. Feasible because byte consumption is independent of handle lookups, so stub lookups give identical framing with no object table, no validation and no GPU. Entire public surface is one question: how long is the command at the start of this slice. BINDING CONSTRAINT: diagnostic and structural only — it may never make a correctness decision, because (c)1 spec §7 relays the ring as opaque bytes precisely so a decode bug cannot become a corruption bug. Task 4 wired the safe Rust wrapper (command_len(), Command, DecodeFault) into rayland-vtest::venus_ring::decode as a fallback behind the fixed-size table — the crate now has its first caller, and rayland-vtest's own tests exercise it (see that node's 'venus_ring decoder' part). Still diagnostic-only end to end: nothing in the workspace lets a decode outcome change what gets relayed. Task 5 will guard that it stays that way.",
      specs: [
