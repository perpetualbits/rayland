//! **Fixture A's end-to-end proof: the same unmodified binary, rendered through the remoting
...
//! It launches `rayland-icosa-cpu` — the CPU fixture: an ordinary offscreen Vulkan program that
//! draws a spinning, fractal-textured icosahedron, writing its texture into a persistently-mapped
//! `HOST_COHERENT` staging buffer every frame with **no flush and no interceptable Vulkan call** —
//! twice, exactly as `refapp_venus_e2e.rs` does for the reference triangle:
//!
...
//! remoted run through the engine — and the CPU-side fractal is computed **bit-exactly** by
//! `rayland-icosa-core`: its `log2`/`sin_cos` are rebuilt from IEEE basic operations precisely so the
//! same program produces identical pixels on any host (see that crate's `exact_math` module). So the
//! **only** thing that differs between the two runs is how the commands — and, for this fixture, the
//! megabyte-per-frame mapped texture write — reached the GPU. Any pixel difference is therefore a
//! defect in that path, which is exactly the assertion this test wants to make.
//!
...
//! upload racing a draw — is invisible to a final-frame check, and is exactly the kind of thing a
...
//! `cargo build -p rayland-icosa-cpu`), never the other way round. `rayland-icosa-cpu` itself still
//! depends on no `rayland-*` crate beyond the two icosa scaffolding crates, and contains no mention
//! of Venus, vtest, or remoting — see that crate's own module doc. The arrow's direction is the
//! whole distinction: the fixture stays a valid, ignorant witness; only this test, standing outside
//! it, knows both halves exist.
//!
//! Run on a GPU host with: `cargo test -p rayland-engine --test icosa_cpu_venus_e2e -- --nocapture`.
...
/// See `refapp_venus_e2e.rs`'s identical constant: this covers Venus initialising and the Vulkan
/// loader enumerating drivers, and exists so a client that never connects at all (the classic
/// `VN_DEBUG=vtest` omission) surfaces as a diagnosable timeout rather than a silent hang.
...
///
/// Fixture A takes ~13.4s natively (roughly 49ms/frame — CPU-side Mandelbrot iteration dominates).
/// Through Venus it is expected to be considerably slower: every frame's megabyte texture write
/// still happens locally (mapped memory is genuinely shared with the engine on this single-machine
/// vtest path), but every Vulkan call in between now crosses the ring, the FFI boundary, and
/// virglrenderer's replay. Ten minutes is generous — an order of magnitude past a plausible slowdown
/// — precisely so a real stall (a fence that never signals, a deadlock in the replay path) is
/// distinguishable from "still working". If this fires, the failure is reported as a **timeout**,
/// not folded into the pixel-comparison failure path, because those are different findings: one says
/// "the path never finished", the other says "the path finished and produced the wrong bytes".
...
/// Build `rayland-icosa-cpu` and return the path to its executable.
///
/// # Why this shells out to Cargo instead of using `CARGO_BIN_EXE_*`
/// Identical reasoning to `refapp_venus_e2e.rs`'s `build_refapp`: Cargo only defines
/// `CARGO_BIN_EXE_<name>` for binaries belonging to the *same package* as the test, and
/// `rayland-icosa-cpu` is a different package. Adding it as a `dev-dependency` of `rayland-engine`
/// does not fix this either — Cargo's `CARGO_BIN_EXE_*` mechanism is defined only for a package's
/// *own* binary targets, not for those of its dependencies, dev or otherwise. So this test asks
/// Cargo for the binary directly, exactly as `build_refapp` does. `cargo build` is a no-op when the
/// binary is already current, which it is under `cargo test --workspace`.
///
/// # Panics
...
fn build_icosa_cpu() -> PathBuf {
    ...
    let status = Command::new(cargo)
        .args(["build", "-p", "rayland-icosa-cpu"])
        .status()
        .expect("cargo must be runnable to build the CPU fixture");
    assert!(status.success(), "building rayland-icosa-cpu must succeed");
    ...
    let fixture = profile_dir.join("rayland-icosa-cpu");
    assert!(
        fixture.is_file(),
        "cargo built rayland-icosa-cpu but no executable is at {}",
        fixture.display()
    );
    fixture
}
...
/// decoded picture unchanged (different compression effort, different chunk ordering, embedded
/// timestamps) and a test that decided pass/fail on decoded pixels would silently paper over that
/// case rather than surface it. Decoding is only how a real pixel difference gets described in terms
/// a human can act on: a byte offset in a *compressed* PNG stream means nothing on its own.
///
/// # Panics
...
/// The headline test: fixture A, run natively and through Rayland, must produce bit-identical PNGs
/// for all 120 frames.
///
/// # Structure
/// Follows `refapp_venus_e2e.rs::refapp_renders_the_same_triangle_through_venus_as_it_does_natively`
/// exactly: this thread plays the server (binds the socket, brings the engine up, launches the
/// fixture as a child process, accepts its connection, serves the vtest protocol), because selecting
/// Venus is done through environment variables read at Vulkan driver-enumeration time, which is far
/// too early to arrange from inside an already-running test process.
///
...
#[test]
fn icosa_cpu_renders_bit_identical_frames_through_venus_as_natively() {
    let node = Path::new(RENDER_NODE);
    if !virgl_available(node) {
        eprintln!(
            "SKIP icosa_cpu_renders_bit_identical_frames_through_venus_as_natively: no usable \
    ...
    if !Path::new(VENUS_ICD).is_file() {
        eprintln!(
            "SKIP icosa_cpu_renders_bit_identical_frames_through_venus_as_natively: no Venus ICD \
             manifest at {VENUS_ICD}"
        );
        return;
    }

    let fixture = build_icosa_cpu();

    // --- The baseline: the same binary, the host's own driver, no Rayland anywhere ---
    let native_dir = std::env::temp_dir().join(format!(
        "rayland-icosa-cpu-e2e-native-{}",
        std::process::id()
    ));
    ...
    let socket_path = PathBuf::from(format!("/tmp/rl-icp-{}.sock", std::process::id()));
    ...
    // including its per-frame megabyte mapped texture write — on the real GPU.
