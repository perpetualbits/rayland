...
> **Merge note (2026-07-27).** The entry immediately below was written on `main` by a parallel
> session while the `wp0-wayland-proxy` branch was running, and the entries after it come from that
> branch (2026-07-22 → 2026-07-27). Both are kept in full — this diary never drops an entry — so the
> dates run out of order across this one seam. Worth reading anyway: that entry is a fresh session
> catching two stale claims in `CLAUDE.md` by reading the project cold, and the branch went on to
> make the same class of mistake several more times.
